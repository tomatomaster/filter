...
function UnsharpMask(rawImage: ImageData): ImageData {
    const pixel: any = rawImage.data;
        ...
        for (let w = 1; w < rawImage.width - 1; w++) {
            ...
            for (let hk = -1; hk < 2; hk++) {
                for (let wk = -1; wk < 2; wk++) {
                    ...
                    const r: number = pixel[position + 0];
                    const g: number = pixel[position + 1];
                    const b: number = pixel[position + 2];
